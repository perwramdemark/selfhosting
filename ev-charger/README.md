...
3. **Current-time dot** (`color-positive`) — slides continuously across the hour via linear interpolation between the two adjacent price points. Rendered as a near-zero-length `<line>` (0.1 viewBox unit) with `stroke-linecap="round"`, `stroke-width="4"`, and `vector-effect="non-scaling-stroke"`, so it stays a circular 4 px dot in screen space despite the stretched viewBox. The tiny non-zero length is deliberate — some browsers cull true zero-length subpaths. The parent `<svg>` uses `overflow="visible"` so the dot's round cap isn't clipped when "now" is near the top or bottom of the viewBox. Skipped if "now" is outside the combined array.
