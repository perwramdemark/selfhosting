...
3. **Current-time dot** (`color-negative`) — slides continuously across the hour via linear interpolation between the two adjacent price points. Rendered as a near-zero-length `<line>` (0.1 viewBox unit) with `stroke-linecap="round"` and `vector-effect="non-scaling-stroke"`, so it stays a circular 6 px dot in screen space despite the stretched viewBox. The tiny non-zero length is deliberate — some browsers cull true zero-length subpaths. Skipped if "now" is outside the combined array.
